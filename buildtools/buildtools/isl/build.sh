...
#
#  description: the script that make install isl
#  date: 2015-8-20
#  version: 1.0
#  history:
#
...
TAR_FILE_NAME=isl-0.18.tar.bz2
SOURCE_CODE_PATH=isl-0.18
LOG_FILE=${LOCAL_DIR}/build_isl.log
BUILD_FAILED=1
...
#######################################################################
log() {
    echo "[Build isl] $(date +%y-%m-%d" "%T): $@"
    echo "[Build isl] $(date +%y-%m-%d" "%T): $@" >> "$LOG_FILE" 2>&1
}
...
function build_component() {
    ...
    for COMPILE_TYPE in ${COMPLIE_TYPE_LIST}; do
        case ${COMPILE_TYPE} in
            release)
                die "[Error] isl not supported build type."
                ;;
            debug)
                die "[Error] isl not supported build type."
                ;;
            comm)
                ...
                log "[Notice] isl configure string: ./configure --prefix=${LOCAL_DIR}/install_comm"
                ./configure --prefix=${LOCAL_DIR}/install_comm --with-gmp-prefix=${LOCAL_DIR}/../gmp/install_comm
                ;;
            release_llt)
                die "[Error] isl not supported build type."
                ;;
        ...
        if [ $? -ne 0 ]; then
            die "[Error] isl configure failed."
        fi
        log "[Notice] isl End configure"

        log "[Notice] isl using \"${COMPILE_TYPE}\" Begin make"
        make
        if [ $? -ne 0 ]; then
            die "isl make failed."
        fi
        log "[Notice] isl End make"

        log "[Notice] isl using \"${COMPILE_TYPE}\" Begin make install"
        make install
        if [ $? -ne 0 ]; then
            die "isl make install failed."
        fi
        log "[Notice] isl End make install"
...
function shrink_component() {
    for COMPILE_TYPE in ${COMPLIE_TYPE_LIST}; do
        ...
        log "[Notice] isl shrink using \"${COMPILE_TYPE}\" has been finished!"
    done
}
...
function dist_component() {
    for COMPILE_TYPE in ${COMPLIE_TYPE_LIST}; do
        ...
        log "[Notice] isl dist using \"${COMPILE_TYPE}\" has been finished!"
    done
}
...
function clean_component() {
    ...
    log "[Notice] isl clean has been finished!"
}
#######################################################################
#######################################################################
#######################################################################

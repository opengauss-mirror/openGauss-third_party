...
# Parameter setting
######################################################################
TAR_FILE_NAME=glog-0.4.0.tar.gz
SOURCE_CODE_PATH=glog-0.4.0
...
function build_component() {
    ...
    for COMPILE_TYPE in ${COMPLIE_TYPE_LIST}; do
        log "[Notice] glog Begin configure..."
        case "${COMPILE_TYPE}" in
            release)
                die "[Error] glog not supported build type."
                ;;
            debug)
                die "[Error] glog not supported build type."
                ;;
            llt)
                ...
                ./autogen.sh
                ./configure CXXFLAGS="-D_GLIBCXX_USE_CXX11_ABI=0  -fPIC" --enable-static=yes --enable-shared=no --prefix=${SCRIPT_PATH}/install_llt
                ;;
            release_llt)
                die "[Error] glog not supported build type."
                ;;
            ...
            comm)
                ...
                ./autogen.sh
                ./configure CXXFLAGS="-D_GLIBCXX_USE_CXX11_ABI=0  -fPIC" --enable-static=yes --enable-shared=no --prefix=${SCRIPT_PATH}/install_comm
                ;;
            *)
        ...
        if [ $? -ne 0 ]; then
            die "[Error] glog configure failed."
        fi
        log "[Notice] glog End configure"

        log_process "[Notice] glog using \"${COMPILE_TYPE}\" Begin make"
        make -j${MAKE_JOBS} >> ${BUILD_LOG_FILE} 2>&1
        if [ $? -ne 0 ]; then
            die "glog make failed."
        fi
        log_process_done

        log_process "[Notice] glog using \"${COMPILE_TYPE}\" Begin make install"
        make install
        if [ $? -ne 0 ]; then
            die "[Error] glog make install failed."
        fi
        log_process_done

        make clean
        log "[Notice] glog build using \"${COMPILE_TYPE}\" has been finished"
    done
}
#######################################################################
...
function shrink_component() {
    for COMPILE_TYPE in ${COMPLIE_TYPE_LIST}; do
        ...
        log "[Notice] glog shrink using \"${COMPILE_TYPE}\" has been finished!"
    done
}
...
function dist_component() {
    for COMPILE_TYPE in ${COMPLIE_TYPE_LIST}; do
        ...
        log "[Notice] glog dist using \"${COMPILE_TYPE}\" has been finished!"
    done
}
...
function clean_component() {
    ...
    log "[Notice] glog clean has been finished!"
}
#######################################################################
#######################################################################
#######################################################################

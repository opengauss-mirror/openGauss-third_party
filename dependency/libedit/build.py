...
# -------------------------------------------------------------------------------#
# usage:                                                                         #
#   python3/python build.py -m all -f libedit-20190324-3.1.tar.gz -t "comm|llt"  #
...
#--------------------------------------------------------#
    ...
    def build_component(self):
        ...
        for c_type in self.compiletype:
            if c_type == 'comm':
                prepare_cmd = 'rm -rf %s/tmp/libedit; mkdir -p %s/tmp/libedit; mkdir -p %s/install/comm' % (self.local_dir, self.local_dir, self.local_dir)
                ret = self.exe_cmd(prepare_cmd)
                self.error_handler(ret)
                config_cmd = 'cd %s/%s; ./configure CC=\'gcc -fPIE -fstack-protector-strong -Wl,-z,relro,-z,now,-z,noexecstack\' --enable-static=yes --enable-shared=no  --prefix=%s/tmp/libedit' % (self.local_dir, source_code_path, self.local_dir)
                ret = self.exe_cmd(config_cmd)
                self.error_handler(ret)
                make_cmd = 'cd %s/%s; make -j4 && make install' % (self.local_dir, source_code_path)
                ret = self.exe_cmd(make_cmd)
                self.error_handler(ret)
                cp_cmd = 'cp -r %s/tmp/libedit/* %s/install/comm; cd %s/%s; make clean' % (self.local_dir, self.local_dir, self.local_dir, source_code_path)
                ret = self.exe_cmd(cp_cmd)
                self.error_handler(ret)
            elif c_type == 'llt':
                prepare_cmd = 'rm -rf %s/tmp/libedit; mkdir -p %s/tmp/libedit; mkdir -p %s/install/llt' % (self.local_dir, self.local_dir, self.local_dir)
                ret = self.exe_cmd(prepare_cmd)
                self.error_handler(ret)
                config_cmd = 'cd %s/%s; ./configure CC=\'gcc -fPIC -std=gnu99 -fstack-protector-strong -Wl,-z,relro,-z,now,-z,noexecstack\' --enable-static=yes --enable-shared=no --prefix=%s/tmp/libedit' % (self.local_dir, source_code_path, self.local_dir)
                ret = self.exe_cmd(config_cmd)
                self.error_handler(ret)
                make_cmd = 'cd %s/%s; make -j4 && make install' % (self.local_dir, source_code_path)
                ret = self.exe_cmd(make_cmd)
                self.error_handler(ret)
                cp_cmd = 'cp -r %s/tmp/libedit/* %s/install/llt; cd %s/%s; make clean' % (self.local_dir, self.local_dir, self.local_dir, source_code_path)
                ret = self.exe_cmd(cp_cmd)
                self.error_handler(ret)
            elif c_type == 'release':
...
#--------------------------------------------------------#
    ...
    def dist_component(self):
        ...
        for binary_str in binary_list:
            install_path = '%s/../../output/dependency/%s/libedit' % (self.local_dir, binary_str)
...
if __name__ == '__main__':
    os.system('python $(pwd)/../../build/pull_open_source.py "libedit" "libedit-20190324-3.1.tar.gz" "05833LPK"')
    args = parse_args()

...
# Parameter setting
######################################################################
TAR_FILE_NAME=zstd-1.4.4.tar.gz
SOURCE_CODE_PATH=zstd-1.4.4
...
function build_component() {
    ...
    for COMPILE_TYPE in ${COMPLIE_TYPE_LIST}; do
        log "[Notice] zstd Begin configure..."
        case "${COMPILE_TYPE}" in
            release)
                die "[Error] zstd not supported build type."
                ;;
            debug)
                die "[Error] zstd not supported build type."
                ;;
            llt)
                ...
                mkdir -p ${SCRIPT_PATH}/${SOURCE_CODE_PATH}/build/cmake/build
                rm -rf ${SCRIPT_PATH}/${SOURCE_CODE_PATH}/build/cmake/build/*
                cd ${SCRIPT_PATH}/${SOURCE_CODE_PATH}/build/cmake/build
                cmake .. -DCMAKE_INSTALL_PREFIX=${SCRIPT_PATH}/install_llt >> ${BUILD_LOG_FILE} 2>&1
                sed -i 's/-std=c99/-std=c99 -fPIC/g' ./programs/CMakeFiles/zstd.dir/flags.make
                sed -i 's/-std=c99/-std=c99 -fPIC/g' ./programs/CMakeFiles/zstd-frugal.dir/flags.make
                sed -i 's/-std=c99/-std=c99 -fPIC/g' ./lib/CMakeFiles/libzstd_static.dir/flags.make
                sed -i 's/-std=c99/-std=c99 -fPIC/g' ./lib/CMakeFiles/libzstd_shared.dir/flags.make
                ;;
            release_llt)
                die "[Error] zstd not supported build type."
                ;;
            ...
            comm)
                ...
                rm -rf ${SCRIPT_PATH}/${SOURCE_CODE_PATH}/build/cmake/build/*
                cd ${SCRIPT_PATH}/${SOURCE_CODE_PATH}/build/cmake/build
                cmake .. -DCMAKE_INSTALL_PREFIX=${SCRIPT_PATH}/install_comm >> ${BUILD_LOG_FILE} 2>&1
                sed -i 's/-std=c99/-std=c99 -fPIC/g' ./programs/CMakeFiles/zstd.dir/flags.make
                sed -i 's/-std=c99/-std=c99 -fPIC/g' ./programs/CMakeFiles/zstd-frugal.dir/flags.make
                sed -i 's/-std=c99/-std=c99 -fPIC/g' ./lib/CMakeFiles/libzstd_static.dir/flags.make
                sed -i 's/-std=c99/-std=c99 -fPIC/g' ./lib/CMakeFiles/libzstd_shared.dir/flags.make
        ...
        if [ $? -ne 0 ]; then
            die "[Error] zstd configure failed."
        fi
        log "[Notice] zstd End configure"

        log_process "[Notice] zstd using \"${COMPILE_TYPE}\" Begin make"
        make -j${MAKE_JOBS} >> ${BUILD_LOG_FILE} 2>&1
        if [ $? -ne 0 ]; then
            die "zstd make failed."
        fi
        log_process_done

        log_process "[Notice] zstd using \"${COMPILE_TYPE}\" Begin make install"
        make install
        if [ $? -ne 0 ]; then
            die "[Error] zstd make install failed."
        fi
        log_process_done

        make clean
        log "[Notice] zstd build using \"${COMPILE_TYPE}\" has been finished"
    done
}
...
function shrink_component() {
    for COMPILE_TYPE in ${COMPLIE_TYPE_LIST}; do
        ...
        log "[Notice] zstd shrink using \"${COMPILE_TYPE}\" has been finished!"
    done
}
...
function dist_component() {
    for COMPILE_TYPE in ${COMPLIE_TYPE_LIST}; do
        ...
        log "[Notice] zstd dist using \"${COMPILE_TYPE}\" has been finished!"
    done
}
...
function clean_component() {
    ...
    log "[Notice] zstd clean has been finished!"
}
#######################################################################
#######################################################################
#######################################################################

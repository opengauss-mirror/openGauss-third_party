    ...
    def build_component(self):
        ...
        for c_type in self.compiletype:
            if c_type == 'release':
                ...
                delete_cmd = "cd %s/%s; sed -i 's/OPTFLAGS= -g -ffast-math/OPTFLAGS= -ffast-math/g' Makefile" % (self.local_dir, source_code_path)
                ret = self.exe_cmd(delete_cmd)
                self.error_handler(ret, delete_cmd)
                config_cmd = "cd %s/%s; ./configure --prefix=%s/install_comm --extra-cflags='-fstack-protector-strong -fPIE -pie -Wl,-z,relro,-z,now -z,noexecstack'" % (self.local_dir, source_code_path, self.local_dir)
...
#--------------------------------------------------------#
    ...
    def dist_component(self):
        ...
        for binary_str in binary_list:
            install_path = '%s/../../output/dependency/%s/fio' % (self.local_dir, binary_str)

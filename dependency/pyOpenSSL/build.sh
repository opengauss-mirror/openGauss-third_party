...
#  description: the script that make install OpenSSL libs
#  date: 2015-9-15
#  version: 1.0
#  history:
...
BUILD_FAILED=1
TAR_FILE_NAME=pyOpenSSL-19.0.0.tar.gz
SOURCE_CODE_PATH=pyOpenSSL-19.0.0
...
#######################################################################
log() {
    echo "[Copy OpenSSL libs] $(date +%y-%m-%d" "%T): $@"
    echo "[Copy OpenSSL libs] $(date +%y-%m-%d" "%T): $@" >> "$LOG_FILE" 2>&1
}
    ...
    for COMPILE_TYPE in ${COMPLIE_TYPE_LIST}; do
        ...
        case "${COMPILE_TYPE}" in
            ...
            debug)
                ...
                ;;
            comm)
                log "[Notice] OpenSSL build string: python setup.py build"
                mkdir -p ${DEPENDENCY_PATH}/install_comm
				export PYTHONPATH=${DEPENDENCY_PATH}/install_comm
				
                python setup.py build
				if [ $? -ne 0 ]; then
					die "[Error] OpenSSL excute cmd python setup.py build failed."
				fi
				
				python setup.py install --install-lib=${DEPENDENCY_PATH}/install_comm
				if [ $? -ne 0 ]; then
					die "[Error] OpenSSL python setup.py install --install-lib=${DEPENDENCY_PATH}/install_comm."
				fi
                ;;
            release_llt)
                die "[Error] OpenSSL not supported build type."
        ...
        if [ $? -ne 0 ]; then
            die "[Error] OpenSSL libs copy failed."
        fi
        log "[Notice] OpenSSL libs End copy"
    done
}
    ...
    for COMPILE_TYPE in ${COMPLIE_TYPE_LIST}; do
        case "${COMPILE_TYPE}" in
            ...
            comm) 
                ...
                cp -rf ${LOCAL_DIR}/${SOURCE_CODE_PATH}/build/lib/OpenSSL/* ${INSTALL_COMPOENT_PATH_NAME}/
                if [ $? -ne 0 ]; then
                    die "[Error] \"cp -rf ${LOCAL_DIR}/${SOURCE_CODE_PATH}/build/lib/OpenSSL/* ${INSTALL_COMPOENT_PATH_NAME}/\" failed."
                fi
                ;;
        ...
        log "[Notice] OpenSSL libs dist using \"${COMPILE_TYPE}\" has been finished!"
    done
}
...
function clean_component() {
    ...
    log "[Notice] OpenSSL libs clean has been finished!"
}
#######################################################################
#######################################################################
#######################################################################

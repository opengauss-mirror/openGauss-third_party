...
#  description: the script that make install pycparser libs
#  date: 2015-9-15
#  version: 1.0
#  history:
...
LOG_FILE=${LOCAL_DIR}/build_pycparser.log
BUILD_FAILED=1
TAR_FILE_NAME=pycparser-2.19.tar.gz
SOURCE_CODE_PATH=pycparser-2.19
...
#######################################################################
log() {
    echo "[Copy pycparser libs] $(date +%y-%m-%d" "%T): $@"
    echo "[Copy pycparser libs] $(date +%y-%m-%d" "%T): $@" >> "$LOG_FILE" 2>&1
}
...
function build_component() {
    ...
    for COMPILE_TYPE in ${COMPLIE_TYPE_LIST}; do
        log "[Notice] pycparser Begin configure..."
        case "${COMPILE_TYPE}" in
            release)
                die "[Error] pycparser not supported build type."
                ;;
            debug)
                die "[Error] pycparser not supported build type."
                ;;
            comm)
                log "[Notice] pycparser build string: python setup.py build"
				mkdir -p ${DEPENDENCY_PATH}/install_comm
				export PYTHONPATH=${DEPENDENCY_PATH}/install_comm
				
                python setup.py build
				if [ $? -ne 0 ]; then
					die "[Error] pycparser excute cmd python setup.py build failed."
				fi
				
				python setup.py install --install-lib=${DEPENDENCY_PATH}/install_comm
				if [ $? -ne 0 ]; then
					die "[Error] pycparser python setup.py install --install-lib=${DEPENDENCY_PATH}/install_comm."
				fi
                ;;
            release_llt)
                die "[Error] pycparser not supported build type."
        ...
        if [ $? -ne 0 ]; then
            die "[Error] pycparser libs copy failed."
        fi
        log "[Notice] pycparser libs End copy"
    done
}
    ...
    for COMPILE_TYPE in ${COMPLIE_TYPE_LIST}; do
        case "${COMPILE_TYPE}" in
            ...
            comm) 
                ...
                cp -rf ${LOCAL_DIR}/${SOURCE_CODE_PATH}/build/lib/pycparser/* ${INSTALL_COMPOENT_PATH_NAME}/
                if [ $? -ne 0 ]; then
                    die "[Error] \"cp -rf ${LOCAL_DIR}/${SOURCE_CODE_PATH}/build/lib/pycparser/* ${INSTALL_COMPOENT_PATH_NAME}/\" failed."
                fi
                ;;
        ...
        log "[Notice] pycparser libs dist using \"${COMPILE_TYPE}\" has been finished!"
    done
}
...
function clean_component() {
    ...
    log "[Notice] pycparser libs clean has been finished!"
}
#######################################################################
#######################################################################
#######################################################################

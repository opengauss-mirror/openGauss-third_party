...
#  description: the script that make install netifaces libs
#  date: 2015-9-15
#  version: 1.0
#  history:
...
LOG_FILE=${LOCAL_DIR}/build_netifaces.log
BUILD_FAILED=1
TAR_FILE_NAME=netifaces-release_0_10_9.tar.gz
SOURCE_CODE_PATH=netifaces-release_0_10_9
...
#######################################################################
log() {
    echo "[Copy netifaces libs] $(date +%y-%m-%d" "%T): $@"
    echo "[Copy netifaces libs] $(date +%y-%m-%d" "%T): $@" >> "$LOG_FILE" 2>&1
}
...
function build_component() {
    ...
    for COMPILE_TYPE in ${COMPLIE_TYPE_LIST}; do
        log "[Notice] netifaces Begin configure..."
        case "${COMPILE_TYPE}" in
            release)
                die "[Error] netifaces not supported build type."
                ;;
            debug)
                die "[Error] netifaces not supported build type."
                ;;
            comm)
                log "[Notice] netifaces build string: python setup.py build"
                mkdir -p ${DEPENDENCY_PATH}/install_comm
				export PYTHONPATH=${DEPENDENCY_PATH}/install_comm
				
                python setup.py build
				if [ $? -ne 0 ]; then
					die "[Error] netifaces excute cmd python setup.py build failed."
				fi
				
				python setup.py install --install-lib=${DEPENDENCY_PATH}/install_comm
				if [ $? -ne 0 ]; then
					die "[Error] netifaces python setup.py install --install-lib=${DEPENDENCY_PATH}/install_comm."
				fi
                ;;
            release_llt)
                die "[Error] netifaces not supported build type."
        ...
        if [ $? -ne 0 ]; then
            die "[Error] netifaces libs copy failed."
        fi
        log "[Notice] netifaces libs End copy"
    done
}
    ...
    for COMPILE_TYPE in ${COMPLIE_TYPE_LIST}; do
        case "${COMPILE_TYPE}" in
            ...
            comm) 
                ...
                rm -f ${INSTALL_COMPOENT_PATH_NAME}/netifaces.so
				rm -f ${INSTALL_COMPOENT_PATH_NAME}/netifaces.so_UCS2
				rm -f ${INSTALL_COMPOENT_PATH_NAME}/netifaces.so_UCS4
                cp -f ${LOCAL_DIR}/${SOURCE_CODE_PATH}/build/lib.*/netifaces.*.so ${INSTALL_COMPOENT_PATH_NAME}/netifaces.so
                if [ $? -ne 0 ]; then
                    die "[Error] \"cp -f ${LOCAL_DIR}/${SOURCE_CODE_PATH}/build/lib.*/netifaces.*.so ${INSTALL_COMPOENT_PATH_NAME}/netifaces.so\" failed."
                fi
				cp -f ${LOCAL_DIR}/${SOURCE_CODE_PATH}/build/lib.*/netifaces.*.so ${INSTALL_COMPOENT_PATH_NAME}/netifaces.so_UCS2
                if [ $? -ne 0 ]; then
                    die "[Error] \"cp -f ${LOCAL_DIR}/${SOURCE_CODE_PATH}/build/lib.*/netifaces.*.so ${INSTALL_COMPOENT_PATH_NAME}/netifaces.so_UCS2\" failed."
                fi
				cp -f ${LOCAL_DIR}/${SOURCE_CODE_PATH}/build/lib.*/netifaces.*.so ${INSTALL_COMPOENT_PATH_NAME}/netifaces.so_UCS4
                if [ $? -ne 0 ]; then
                    die "[Error] \"cp -f ${LOCAL_DIR}/${SOURCE_CODE_PATH}/build/lib.*/netifaces.*.so ${INSTALL_COMPOENT_PATH_NAME}/netifaces.so_UCS4\" failed."
                fi
                ;;
        ...
        log "[Notice] netifaces libs dist using \"${COMPILE_TYPE}\" has been finished!"
    done
}
...
function clean_component() {
    ...
    log "[Notice] netifaces libs clean has been finished!"
}
#######################################################################
#######################################################################
#######################################################################

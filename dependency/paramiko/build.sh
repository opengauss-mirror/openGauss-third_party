...
#  description: the script that make install paramiko libs
#  date: 2015-9-15
#  version: 1.0
#  history:
...
LOG_FILE=${LOCAL_DIR}/build_paramiko.log
BUILD_FAILED=1
TAR_FILE_NAME=paramiko-2.6.0.tar.gz
SOURCE_CODE_PATH=paramiko-2.6.0
...
#######################################################################
log() {
    echo "[Copy paramiko libs] $(date +%y-%m-%d" "%T): $@"
    echo "[Copy paramiko libs] $(date +%y-%m-%d" "%T): $@" >> "$LOG_FILE" 2>&1
}
...
function build_component() {
    ...
    for COMPILE_TYPE in ${COMPLIE_TYPE_LIST}; do
        log "[Notice] paramiko Begin configure..."
        case "${COMPILE_TYPE}" in
            release)
                die "[Error] paramiko not supported build type."
                ;;
            debug)
                die "[Error] paramiko not supported build type."
                ;;
            comm)
                log "[Notice] paramiko build string: python setup.py build"
                mkdir -p ${DEPENDENCY_PATH}/install_comm
				export PYTHONPATH=${DEPENDENCY_PATH}/install_comm
				
                python setup.py build
				if [ $? -ne 0 ]; then
					die "[Error] paramiko excute cmd python setup.py build failed."
				fi
				
				python setup.py install --install-lib=${DEPENDENCY_PATH}/install_comm
				if [ $? -ne 0 ]; then
					die "[Error] paramiko python setup.py install --install-lib=${DEPENDENCY_PATH}/install_comm."
				fi
                ;;
            release_llt)
                die "[Error] paramiko not supported build type."
        ...
        if [ $? -ne 0 ]; then
            die "[Error] paramiko libs copy failed."
        fi
        log "[Notice] paramiko libs End copy"
    done
}
    ...
    for COMPILE_TYPE in ${COMPLIE_TYPE_LIST}; do
        case "${COMPILE_TYPE}" in
            ...
            comm) 
                ...
                cp -rf ${LOCAL_DIR}/${SOURCE_CODE_PATH}/build/lib/paramiko/* ${INSTALL_COMPOENT_PATH_NAME}/
                if [ $? -ne 0 ]; then
                    die "[Error] \"cp -rf ${LOCAL_DIR}/${SOURCE_CODE_PATH}/build/lib/paramiko/* ${INSTALL_COMPOENT_PATH_NAME}/\" failed."
                fi
                ;;
        ...
        log "[Notice] paramiko libs dist using \"${COMPILE_TYPE}\" has been finished!"
    done
}
...
function clean_component() {
    ...
    log "[Notice] paramiko libs clean has been finished!"
}
#######################################################################
#######################################################################
#######################################################################

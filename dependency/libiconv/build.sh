...
# Parameter setting
######################################################################
TAR_FILE_NAME=libiconv-1.15.tar.gz
SOURCE_CODE_PATH=libiconv-1.15
...
function build_component() {
    ...
    for COMPILE_TYPE in ${COMPLIE_TYPE_LIST}; do
        log "[Notice] libiconv Begin configure..."
        case "${COMPILE_TYPE}" in
            release)
                die "[Error] libiconv not supported build type."
                ;;
            debug)
                die "[Error] libiconv not supported build type."
                ;;
        ...
        if [ $? -ne 0 ]; then
            die "[Error] libiconv configure failed."
        fi
        log "[Notice] libiconv End configure"

        log_process "[Notice] libiconv using \"${COMPILE_TYPE}\" Begin make"
        make -j >> ${BUILD_LOG_FILE} 2>&1
        if [ $? -ne 0 ]; then
            die "libiconv make failed."
        fi
        log_process_done

        log_process "[Notice] libiconv using \"${COMPILE_TYPE}\" Begin make install"
        make install
        if [ $? -ne 0 ]; then
            die "[Error] libiconv make install failed."
        fi
        log_process_done

        make clean
        log "[Notice] libiconv build using \"${COMPILE_TYPE}\" has been finished"
    done
}
...
function shrink_component() {
    for COMPILE_TYPE in ${COMPLIE_TYPE_LIST}; do
        ...
        log "[Notice] libiconv shrink using \"${COMPILE_TYPE}\" has been finished!"
    done
}
...
function dist_component() {
    for COMPILE_TYPE in ${COMPLIE_TYPE_LIST}; do
        ...
        log "[Notice] libiconv dist using \"${COMPILE_TYPE}\" has been finished!"
    done
}
...
function clean_component() {
    ...
    log "[Notice] libiconv clean has been finished!"
}
#######################################################################
#######################################################################
#######################################################################

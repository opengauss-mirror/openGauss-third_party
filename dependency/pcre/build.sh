...
# Parameter setting
######################################################################
TAR_FILE_NAME=pcre-8.42.tar.gz
SOURCE_CODE_PATH=pcre-8.42
...
function build_component() {
    ...
    for COMPILE_TYPE in ${COMPLIE_TYPE_LIST}; do
        log "[Notice] pcre Begin configure..."
        case "${COMPILE_TYPE}" in
            release)
                die "[Error] pcre not supported build type."
                ;;
            debug)
                die "[Error] pcre not supported build type."
                ;;
            llt)
                ...
                ./configure CFLAGS='-fPIC -fstack-protector-all --param ssp-buffer-size=4 -Wstack-protector' CPPFLAGS='-fPIC -fstack-protector-all --param ssp-buffer-size=4 -Wstack-protector' LDFLAGS='-Wl,-z,relro,-z,now' --prefix=${LIB_ROOT_PATH}/pcre/install_llt --enable-utf --disable-rpath >> $LOG_FILE 2>&1
                ;;
            release_llt)
                die "[Error] pcre not supported build type."
                ;;
            ...
            comm)
                ...
                ./configure CFLAGS='-fPIC -fstack-protector-all --param ssp-buffer-size=4 -Wstack-protector' CPPFLAGS='-fPIC -fstack-protector-all --param ssp-buffer-size=4 -Wstack-protector' LDFLAGS='-Wl,-z,relro,-z,now' --prefix=${LIB_ROOT_PATH}/pcre/install_comm --enable-utf --disable-rpath >> $LOG_FILE 2>&1
                ;;
            *)
        ...
        if [ $? -ne 0 ]; then
            die "[Error] pcre configure failed."
        fi
        log "[Notice] pcre End configure"

        log_process "[Notice] pcre using \"${COMPILE_TYPE}\" Begin make"
        make -j >> ${BUILD_LOG_FILE} 2>&1
        if [ $? -ne 0 ]; then
            die "pcre make failed."
        fi
        log_process_done

        log_process "[Notice] pcre using \"${COMPILE_TYPE}\" Begin make install"
        make install
        if [ $? -ne 0 ]; then
            die "[Error] pcre make install failed."
        fi
        log_process_done

        make clean
        log "[Notice] pcre build using \"${COMPILE_TYPE}\" has been finished"
    done
}
...
function shrink_component() {
    for COMPILE_TYPE in ${COMPLIE_TYPE_LIST}; do
        ...
        log "[Notice] pcre shrink using \"${COMPILE_TYPE}\" has been finished!"
    done
}
...
function dist_component() {
    for COMPILE_TYPE in ${COMPLIE_TYPE_LIST}; do
        ...
        log "[Notice] pcre dist using \"${COMPILE_TYPE}\" has been finished!"
    done
}
...
function clean_component() {
    ...
    log "[Notice] pcre clean has been finished!"
}
#######################################################################
#######################################################################
#######################################################################

...
#  description: the script that make install nacl libs
#  date: 2015-9-15
#  version: 1.0
#  history:
...
LOG_FILE=${LOCAL_DIR}/build_nacl.log
BUILD_FAILED=1
TAR_FILE_NAME=pynacl-1.3.0.tar.gz
SOURCE_CODE_PATH=pynacl-1.3.0
...
#######################################################################
log() {
    echo "[Copy nacl libs] $(date +%y-%m-%d" "%T): $@"
    echo "[Copy nacl libs] $(date +%y-%m-%d" "%T): $@" >> "$LOG_FILE" 2>&1
}
...
function build_component() {
    ...
    for COMPILE_TYPE in ${COMPLIE_TYPE_LIST}; do
        log "[Notice] nacl Begin configure..."
        case "${COMPILE_TYPE}" in
            release)
                die "[Error] nacl not supported build type."
                ;;
            debug)
                die "[Error] nacl not supported build type."
                ;;
            comm)
                log "[Notice] nacl build string: python setup.py build"
                mkdir -p ${DEPENDENCY_PATH}/install_comm
				export PYTHONPATH=${DEPENDENCY_PATH}/install_comm
				
                python setup.py build
				if [ $? -ne 0 ]; then
					die "[Error] nacl excute cmd python setup.py build failed."
				fi
				
				python setup.py install --install-lib=${DEPENDENCY_PATH}/install_comm
				if [ $? -ne 0 ]; then
					die "[Error] nacl python setup.py install --install-lib=${DEPENDENCY_PATH}/install_comm."
				fi
                ;;
            release_llt)
                die "[Error] nacl not supported build type."
        ...
        if [ $? -ne 0 ]; then
            die "[Error] nacl libs copy failed."
        fi
        log "[Notice] nacl libs End copy"
    done
}

function create_bootstarp_file() {
	cp -f ${DEPENDENCY_PATH}/install_comm/PyNaCl-*.egg/nacl/_sodium.py ${INSTALL_COMPOENT_PATH_NAME}/
	if [ $? -ne 0 ]; then
        die "[Error] \"cp -f ${DEPENDENCY_PATH}/install_comm/PyNaCl-*.egg/nacl/_sodium.py ${INSTALL_COMPOENT_PATH_NAME}/\" failed."
    fi
	sed -i "s#_sodium.abi3.so#_sodium.so#g" ${INSTALL_COMPOENT_PATH_NAME}/_sodium.py
	if [ $? -ne 0 ]; then
        die "[Error] \"modify ${INSTALL_COMPOENT_PATH_NAME}/_sodium.py\" failed."
    fi
}
    ...
    for COMPILE_TYPE in ${COMPLIE_TYPE_LIST}; do
        case "${COMPILE_TYPE}" in
            ...
            comm) 
                ...
                cp -rf ${LOCAL_DIR}/${SOURCE_CODE_PATH}/build/lib.*/nacl/* ${INSTALL_COMPOENT_PATH_NAME}/
                if [ $? -ne 0 ]; then
                    die "[Error] \"cp -rf ${LOCAL_DIR}/${SOURCE_CODE_PATH}/build/lib.*/nacl/* ${INSTALL_COMPOENT_PATH_NAME}/\" failed."
                fi
				mv ${INSTALL_COMPOENT_PATH_NAME}/_sodium.abi3.so ${INSTALL_COMPOENT_PATH_NAME}/_sodium.so
                if [ $? -ne 0 ]; then
                    die "[Error] \"mv ${INSTALL_COMPOENT_PATH_NAME}/_sodium.abi3.so ${INSTALL_COMPOENT_PATH_NAME}/_sodium.so\" failed."
                fi
        ...
        log "[Notice] nacl libs dist using \"${COMPILE_TYPE}\" has been finished!"
    done
}
...
function clean_component() {
    ...
    log "[Notice] nacl libs clean has been finished!"
}
#######################################################################
#######################################################################
#######################################################################

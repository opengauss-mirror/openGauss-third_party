...
# Parameter setting
######################################################################
TAR_FILE_NAME=brotli-0.6.0.tar.gz
SOURCE_CODE_PATH=brotli-0.6.0
...
function build_component() {
    ...
    mkdir ${SCRIPT_PATH}/${SOURCE_CODE_PATH}/build

    cd ${SCRIPT_PATH}/${SOURCE_CODE_PATH}/build

    for COMPILE_TYPE in ${COMPLIE_TYPE_LIST}; do
        log "[Notice] brotil Begin configure..."
        case "${COMPILE_TYPE}" in
            release)
                die "[Error] brotil not supported build type."
                ;;
            debug)
                die "[Error] brotil not supported build type."
                ;;
            llt)
                ../configure-cmake --disable-shared-libs --disable-debug --prefix=${SCRIPT_PATH}/install_llt
                ;;
            release_llt)
                die "[Error] brotil not supported build type."
                ;;
            debug_llt)
                ...
                ;;
            comm)
                ../configure-cmake --disable-shared-libs --disable-debug --prefix=${SCRIPT_PATH}/install_comm
                ;;
            *)
        ...
        if [ $? -ne 0 ]; then
            die "[Error] brotil configure failed."
        fi
        log "[Notice] brotil End configure"

        log_process "[Notice] brotil using \"${COMPILE_TYPE}\" Begin make"
        make -j >> ${BUILD_LOG_FILE} 2>&1
        if [ $? -ne 0 ]; then
            die "brotil make failed."
        fi
        log_process_done

        log_process "[Notice] brotil using \"${COMPILE_TYPE}\" Begin make install"
        make install
        if [ $? -ne 0 ]; then
            die "[Error] brotil make install failed."
        fi
        log_process_done

        make clean
        log "[Notice] brotil build using \"${COMPILE_TYPE}\" has been finished"
    done
}
...
function shrink_component() {
    for COMPILE_TYPE in ${COMPLIE_TYPE_LIST}; do
        ...
        log "[Notice] brotil shrink using \"${COMPILE_TYPE}\" has been finished!"
    done
}
...
function dist_component() {
    for COMPILE_TYPE in ${COMPLIE_TYPE_LIST}; do
        ...
        log "[Notice] brotil dist using \"${COMPILE_TYPE}\" has been finished!"
    done
}
...
function clean_component() {
    ...
    log "[Notice] brotil clean has been finished!"
}
#######################################################################
#######################################################################
#######################################################################

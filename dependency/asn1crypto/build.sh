...
#  description: the script that make install asn1crypto libs
#  date: 2015-9-15
#  version: 1.0
#  history:
...
LOG_FILE=${LOCAL_DIR}/build_asn1crypto.log
BUILD_FAILED=1
TAR_FILE_NAME=asn1crypto-1.2.0.tar.gz
SOURCE_CODE_PATH=asn1crypto-1.2.0
...
#######################################################################
log() {
    echo "[Copy asn1crypto libs] $(date +%y-%m-%d" "%T): $@"
    echo "[Copy asn1crypto libs] $(date +%y-%m-%d" "%T): $@" >> "$LOG_FILE" 2>&1
}
...
function build_component() {
    ...
    for COMPILE_TYPE in ${COMPLIE_TYPE_LIST}; do
        log "[Notice] asn1crypto Begin configure..."
        case "${COMPILE_TYPE}" in
            release)
                die "[Error] asn1crypto not supported build type."
                ;;
            debug)
                die "[Error] asn1crypto not supported build type."
                ;;
            comm)
                log "[Notice] asn1crypto build string: python setup.py build"
                mkdir -p ${DEPENDENCY_PATH}/install_comm
				export PYTHONPATH=${DEPENDENCY_PATH}/install_comm
				
                python setup.py build
				if [ $? -ne 0 ]; then
					die "[Error] asn1crypto excute cmd python setup.py build failed."
				fi
				
				python setup.py install --install-lib=${DEPENDENCY_PATH}/install_comm
				if [ $? -ne 0 ]; then
					die "[Error] asn1crypto python setup.py install --install-lib=${DEPENDENCY_PATH}/install_comm."
				fi
                ;;
            release_llt)
                die "[Error] asn1crypto not supported build type."
        ...
        if [ $? -ne 0 ]; then
            die "[Error] asn1crypto libs copy failed."
        fi
        log "[Notice] asn1crypto libs End copy"
    done
}
    ...
    for COMPILE_TYPE in ${COMPLIE_TYPE_LIST}; do
        case "${COMPILE_TYPE}" in
            ...
            comm) 
                ...
                cp -rf ${LOCAL_DIR}/${SOURCE_CODE_PATH}/build/lib/asn1crypto/* ${INSTALL_COMPOENT_PATH_NAME}/
                if [ $? -ne 0 ]; then
                    die "[Error] \"cp -rf ${LOCAL_DIR}/${SOURCE_CODE_PATH}/build/* ${INSTALL_COMPOENT_PATH_NAME}/\" failed."
                fi
                ;;
        ...
        log "[Notice] asn1crypto libs dist using \"${COMPILE_TYPE}\" has been finished!"
    done
}
...
function clean_component() {
    ...
    log "[Notice] asn1crypto libs clean has been finished!"
}
#######################################################################
#######################################################################
#######################################################################

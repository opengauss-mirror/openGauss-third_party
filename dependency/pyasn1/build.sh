...
#  description: the script that make install pyasn1 libs
#  date: 2015-9-15
#  version: 1.0
#  history:
...
LOG_FILE=${LOCAL_DIR}/build_pyasn1.log
BUILD_FAILED=1
TAR_FILE_NAME=pyasn1-0.4.8.tar.gz 
SOURCE_CODE_PATH=pyasn1-0.4.8
...
#######################################################################
log() {
    echo "[Copy pyasn1 libs] $(date +%y-%m-%d" "%T): $@"
    echo "[Copy pyasn1 libs] $(date +%y-%m-%d" "%T): $@" >> "$LOG_FILE" 2>&1
}
...
function build_component() {
    ...
    for COMPILE_TYPE in ${COMPLIE_TYPE_LIST}; do
        log "[Notice] pyasn1 Begin configure..."
        case "${COMPILE_TYPE}" in
            release)
                die "[Error] pyasn1 not supported build type."
                ;;
            debug)
                die "[Error] pyasn1 not supported build type."
                ;;
            comm)
                log "[Notice] pyasn1 build string: python setup.py build"
                mkdir -p ${DEPENDENCY_PATH}/install_comm
				export PYTHONPATH=${DEPENDENCY_PATH}/install_comm
				
                python setup.py build
				if [ $? -ne 0 ]; then
					die "[Error] pyasn1 excute cmd python setup.py build failed."
				fi
				
				python setup.py install --install-lib=${DEPENDENCY_PATH}/install_comm
				if [ $? -ne 0 ]; then
					die "[Error] pyasn1 python setup.py install --install-lib=${DEPENDENCY_PATH}/install_comm."
				fi
                ;;
            release_llt)
                die "[Error] pyasn1 not supported build type."
        ...
        if [ $? -ne 0 ]; then
            die "[Error] pyasn1 libs copy failed."
        fi
        log "[Notice] pyasn1 libs End copy"
    done
}
    ...
    for COMPILE_TYPE in ${COMPLIE_TYPE_LIST}; do
        case "${COMPILE_TYPE}" in
            ...
            comm) 
                ...
                cp -rf ${LOCAL_DIR}/${SOURCE_CODE_PATH}/build/lib/pyasn1/* ${INSTALL_COMPOENT_PATH_NAME}/
                if [ $? -ne 0 ]; then
                    die "[Error] \"cp -rf ${LOCAL_DIR}/${SOURCE_CODE_PATH}/build/lib/pyasn1/* ${INSTALL_COMPOENT_PATH_NAME}/\" failed."
                fi
                ;;
        ...
        log "[Notice] pyasn1 libs dist using \"${COMPILE_TYPE}\" has been finished!"
    done
}
...
function clean_component() {
    ...
    log "[Notice] pyasn1 libs clean has been finished!"
}
#######################################################################
#######################################################################
#######################################################################

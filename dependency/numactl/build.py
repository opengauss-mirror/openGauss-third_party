...
#-------------------------------------------------------------------------#
#usage:                                                                   #
#   python3/python build.py -m all -f xxx.tar.gz -t "comm" #
...
#--------------------------------------------------------#
    ...
    def binary_parser(self):
        ...
        if platform_str == 'euleros2.0_sp5_x86_64':
            print("[LOG] Not supported platform type")
    ...
    def build_component(self):
        ...
        for c_type in self.compiletype:
            if c_type == 'comm':
                ...
                self.error_handler(ret)
                config_cmd = "cd %s/%s; ./configure CFLAGS='-fstack-protector-all -Wl,-z,relro,-z,now -Wl,-z,noexecstack -fPIC' --prefix=%s/install/comm" % (self.local_dir, source_code_path, self.local_dir)
                ...
                ret = self.exe_cmd(prepare_cmd)
                self.error_handler(ret)
                config_cmd = "cd %s/%s; ./configure CFLAGS='-fstack-protector-all -Wl,-z,relro,-z,now -Wl,-z,noexecstack -fPIC' --prefix=%s/install/llt" % (self.local_dir, source_code_path, self.local_dir)
                ret = self.exe_cmd(config_cmd)
                self.error_handler(ret)
                make_cmd = 'cd %s/%s; make -j4 && make install' % (self.local_dir, source_code_path)
                ret = self.exe_cmd(make_cmd)
...
#--------------------------------------------------------#
    ...
    def shrink_component(self):
        ...
        install_dist_cmd = 'cd %s && mkdir -p install_comm_dist && rm -rf install_comm_dist/* && mkdir -p install_comm_dist/include && mkdir -p install_comm_dist/lib' % (self.local_dir)
        ret = self.exe_cmd(install_dist_cmd)
        self.error_handler(ret)
        for c_type in self.compiletype:
            if c_type == 'comm':
                cp_cmd1 = 'cp -r %s/install/comm/include/* %s/install_comm_dist/include' % (self.local_dir, self.local_dir)
                self.exe_cmd(cp_cmd1)
                cp_cmd2 = 'cp -r %s/install/comm/lib/*so* %s/install_comm_dist/lib' % (self.local_dir, self.local_dir)
                self.exe_cmd(cp_cmd2)
            elif c_type == 'llt':
                cp_cmd1 = 'cp -r %s/install/llt/include/* %s/install_comm_dist/include' % (self.local_dir, self.local_dir)
                self.exe_cmd(cp_cmd1)
                cp_cmd2 = 'cp -r %s/install/llt/lib/*so* %s/install_comm_dist/lib' % (self.local_dir, self.local_dir)
                self.exe_cmd(cp_cmd2)
            elif c_type == 'release':
                print ("[WARNING] Not supported build type")
            elif c_type == 'debug':
...
#--------------------------------------------------------#
    ...
    def dist_component(self):
        ...
        for binary_str in binary_list:
            install_path = '%s/../../output/dependency/%s/numactl' % (self.local_dir, binary_str)
            # move source code type
            for c_type in self.compiletype:
                if c_type == 'comm':
                    #rm_cmd = 'cd %s && mkdir -p comm && rm -rf comm/*' % (install_path)
                    rm_cmd = 'rm -rf %s/comm; mkdir -p %s/comm' % (install_path, install_path)
                    self.exe_cmd(rm_cmd)
                    cp_cmd2 = 'cp -r %s/install_comm_dist/include %s/comm/' % (self.local_dir, install_path)
                    self.exe_cmd(cp_cmd2)
                    cp_cmd3 = 'cp -rL %s/install_comm_dist/lib %s/comm/' % (self.local_dir, install_path)
                    self.exe_cmd(cp_cmd3)
                elif c_type == 'llt':
                    #rm_cmd = 'cd %s && mkdir -p llt && rm -rf llt/*' % (install_path)
                    rm_cmd = 'rm -rf %s/llt; mkdir -p %s/llt' % (install_path, install_path)
                    self.exe_cmd(rm_cmd)
                    cp_cmd2 = 'cp -r %s/install_comm_dist/include %s/llt/' % (self.local_dir, install_path)
                    self.exe_cmd(cp_cmd2)
                    cp_cmd3 = 'cp -rL %s/install_comm_dist/lib %s/llt/' % (self.local_dir, install_path)
                    self.exe_cmd(cp_cmd3)
                elif c_type == 'release':
                    print ("[WARNING] Not supported build type")
                elif c_type == 'debug':
...
#--------------------------------------------------------#
    ...
    def clean_component(self):
        ...
        ret = self.exe_cmd(clean_cmd)
        self.error_handler(ret)
        rm_cmd = 'cd %s; rm -rf %s; rm -rf install; rm -rf install_comm_dist' % (self.local_dir, source_code_path)
...
if __name__ == '__main__':
    ...
    cmd_str='python $(pwd)/../../build/pull_open_source.py "numactl" "numactl-2.0.13.tar.gz" "05833MQM"'

...
#  description: the script that make install bcrypt libs
#  date: 2015-9-15
#  version: 1.0
#  history:
...
LOG_FILE=${LOCAL_DIR}/build_bcrypt.log
BUILD_FAILED=1
TAR_FILE_NAME=bcrypt-3.1.7.tar.gz
SOURCE_CODE_PATH=bcrypt-3.1.7
...
#######################################################################
log() {
    echo "[Copy bcrypt libs] $(date +%y-%m-%d" "%T): $@"
    echo "[Copy bcrypt libs] $(date +%y-%m-%d" "%T): $@" >> "$LOG_FILE" 2>&1
}
...
function build_component() {
    ...
    for COMPILE_TYPE in ${COMPLIE_TYPE_LIST}; do
        log "[Notice] bcrypt Begin configure..."
        case "${COMPILE_TYPE}" in
            release)
                die "[Error] bcrypt not supported build type."
                ;;
            debug)
                die "[Error] bcrypt not supported build type."
                ;;
            comm)
                log "[Notice] bcrypt build string: python setup.py build"
				mkdir -p ${DEPENDENCY_PATH}/install_comm
				export PYTHONPATH=${DEPENDENCY_PATH}/install_comm
				
                python setup.py build
				if [ $? -ne 0 ]; then
					die "[Error] bcrypt excute cmd python setup.py build failed."
				fi
				
				python setup.py install --install-lib=${DEPENDENCY_PATH}/install_comm
				if [ $? -ne 0 ]; then
					die "[Error] bcrypt python setup.py install --install-lib=${DEPENDENCY_PATH}/install_comm."
				fi
                ;;
            release_llt)
                die "[Error] bcrypt not supported build type."
        ...
        if [ $? -ne 0 ]; then
            die "[Error] bcrypt libs copy failed."
        fi
        log "[Notice] bcrypt libs End copy"
    done
}

function create_bootstarp_file() {
	cp -f ${DEPENDENCY_PATH}/install_comm/bcrypt-*.egg/bcrypt/_bcrypt.py ${INSTALL_COMPOENT_PATH_NAME}/
	if [ $? -ne 0 ]; then
        die "[Error] \"cp -f ${DEPENDENCY_PATH}/install_comm/bcrypt-*.egg/bcrypt/_bcrypt.py ${INSTALL_COMPOENT_PATH_NAME}/\" failed."
    fi
	sed -i "s#_bcrypt.abi3.so#_bcrypt.so#g" ${INSTALL_COMPOENT_PATH_NAME}/_bcrypt.py
	if [ $? -ne 0 ]; then
        die "[Error] \"modify ${INSTALL_COMPOENT_PATH_NAME}/../_bcrypt.py\" failed."
    fi
}
    ...
    for COMPILE_TYPE in ${COMPLIE_TYPE_LIST}; do
        case "${COMPILE_TYPE}" in
            ...
            comm) 
                ...
                cp -rf ${LOCAL_DIR}/${SOURCE_CODE_PATH}/build/lib.*/bcrypt/* ${INSTALL_COMPOENT_PATH_NAME}/
                if [ $? -ne 0 ]; then
                    die "[Error] \"cp -rf ${LOCAL_DIR}/${SOURCE_CODE_PATH}/build/lib.*/bcrypt/* ${INSTALL_COMPOENT_PATH_NAME}/\" failed."
                fi
				mv ${INSTALL_COMPOENT_PATH_NAME}/_bcrypt.abi3.so ${INSTALL_COMPOENT_PATH_NAME}/_bcrypt.so
				if [ $? -ne 0 ]; then
                    die "[Error] \"mv ${INSTALL_COMPOENT_PATH_NAME}/_bcrypt.abi3.so ${INSTALL_COMPOENT_PATH_NAME}/_bcrypt.so\" failed."
                fi
        ...
        log "[Notice] bcrypt libs dist using \"${COMPILE_TYPE}\" has been finished!"
    done
}
...
function clean_component() {
    ...
    log "[Notice] bcrypt libs clean has been finished!"
}
#######################################################################
#######################################################################
#######################################################################

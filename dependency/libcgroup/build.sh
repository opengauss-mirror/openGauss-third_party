...
# the script that make install libcgroup
# version: 1.0.0
# note: yum install pam-devel
# change log:
# ***********************************************************************
set -e
...
# Parameter setting
######################################################################
RPM_FILE_NAME=libcgroup-0.41-21.el7.src.rpm
TAR_FILE_NAME=libcgroup-0.41.tar.bz2
SOURCE_CODE_PATH=libcgroup-0.41
...
function build_component() {
    ...
    rpm2cpio libcgroup-0.41-21.el7.src.rpm | cpio -div
    tar -xvf ${TAR_FILE_NAME} > ${LOG_FILE}

    patch_for_build p0 libcgroup.patch

    cd ${SCRIPT_PATH}/${SOURCE_CODE_PATH}

    for COMPILE_TYPE in ${COMPLIE_TYPE_LIST}; do
        log "[Notice] libcgroup Begin configure..."
        case "${COMPILE_TYPE}" in
            release)
                die "[Error] libcgroup not supported build type."
                ;;
            debug)
                die "[Error] libcgroup not supported build type."
                ;;
            llt)
                ...
                ./configure CFLAGS='-fstack-protector-all -Wl,-z,relro,-z,now -Wl,-z,noexecstack -fPIC' --enable-static=no --enable-shared=yes --prefix=${SCRIPT_PATH}/install_llt
                ;;
            release_llt)
                die "[Error] libcgroup not supported build type."
                ;;
            ...
            comm)
                ...
                ./configure CFLAGS='-fstack-protector-all -Wl,-z,relro,-z,now -Wl,-z,noexecstack -fPIC' --enable-static=no --enable-shared=yes --prefix=${SCRIPT_PATH}/install_comm
                ;;
            *)
        ...
        if [ $? -ne 0 ]; then
            die "[Error] libcgroup configure failed."
        fi
        log "[Notice] libcgroup End configure"

        log_process "[Notice] libcgroup using \"${COMPILE_TYPE}\" Begin make"
        make -j >> ${BUILD_LOG_FILE} 2>&1
        if [ $? -ne 0 ]; then
            die "libcgroup make failed."
        fi
        log_process_done

        log_process "[Notice] libcgroup using \"${COMPILE_TYPE}\" Begin make install"
        make install
        if [ $? -ne 0 ]; then
            die "[Error] libcgroup make install failed."
        fi
        log_process_done

        make clean
        log "[Notice] libcgroup build using \"${COMPILE_TYPE}\" has been finished"
    done
}
...
function shrink_component() {
    for COMPILE_TYPE in ${COMPLIE_TYPE_LIST}; do
        ...
        log "[Notice] libcgroup shrink using \"${COMPILE_TYPE}\" has been finished!"
    done
}
...
function dist_component() {
    for COMPILE_TYPE in ${COMPLIE_TYPE_LIST}; do
        ...
        log "[Notice] libcgroup dist using \"${COMPILE_TYPE}\" has been finished!"
    done
}
...
function clean_component() {
    ...
    log "[Notice] libcgroup clean has been finished!"
}
#######################################################################
#######################################################################
#######################################################################

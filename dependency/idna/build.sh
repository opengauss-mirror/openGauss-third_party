...
#  description: the script that make install idna libs
#  date: 2015-9-15
#  version: 1.0
#  history:
...
LOG_FILE=${LOCAL_DIR}/build_idna.log
BUILD_FAILED=1
TAR_FILE_NAME=idna-2.9.tar.gz
SOURCE_CODE_PATH=idna-2.9
...
#######################################################################
log() {
    echo "[Copy idna libs] $(date +%y-%m-%d" "%T): $@"
    echo "[Copy idna libs] $(date +%y-%m-%d" "%T): $@" >> "$LOG_FILE" 2>&1
}
...
function build_component() {
    ...
    for COMPILE_TYPE in ${COMPLIE_TYPE_LIST}; do
        log "[Notice] idna Begin configure..."
        case "${COMPILE_TYPE}" in
            release)
                die "[Error] idna not supported build type."
                ;;
            debug)
                die "[Error] idna not supported build type."
                ;;
            comm)
                log "[Notice] idna build string: python setup.py build"
                mkdir -p ${DEPENDENCY_PATH}/install_comm
				export PYTHONPATH=${DEPENDENCY_PATH}/install_comm
				
                python setup.py build
				if [ $? -ne 0 ]; then
					die "[Error] idna excute cmd python setup.py build failed."
				fi
				
				python setup.py install --install-lib=${DEPENDENCY_PATH}/install_comm
				if [ $? -ne 0 ]; then
					die "[Error] idna python setup.py install --install-lib=${DEPENDENCY_PATH}/install_comm."
				fi
                ;;
            release_llt)
                die "[Error] idna not supported build type."
        ...
        if [ $? -ne 0 ]; then
            die "[Error] idna libs copy failed."
        fi
        log "[Notice] idna libs End copy"
    done
}
    ...
    for COMPILE_TYPE in ${COMPLIE_TYPE_LIST}; do
        case "${COMPILE_TYPE}" in
            ...
            comm) 
                ...
                cp -rf ${LOCAL_DIR}/${SOURCE_CODE_PATH}/build/lib/idna/* ${INSTALL_COMPOENT_PATH_NAME}/
                if [ $? -ne 0 ]; then
                    die "[Error] \"cp -rf ${LOCAL_DIR}/${SOURCE_CODE_PATH}/build/lib/idna/* ${INSTALL_COMPOENT_PATH_NAME}/\" failed."
                fi
                ;;
        ...
        log "[Notice] idna libs dist using \"${COMPILE_TYPE}\" has been finished!"
    done
}
...
function clean_component() {
    ...
    log "[Notice] idna libs clean has been finished!"
}
#######################################################################
#######################################################################
#######################################################################

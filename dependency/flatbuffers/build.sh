...
# Parameter setting
######################################################################
ZIP_FILE_NAME=flatbuffers-1.11.0-src.zip
TAR_FILE_NAME=flatbuffers-1.11.0.tar.gz
SOURCE_CODE_PATH=flatbuffers-1.11.0
...
function build_component() {
    ...
    for COMPILE_TYPE in ${COMPLIE_TYPE_LIST}; do
        log "[Notice] flatbuffers Begin configure..."
        case "${COMPILE_TYPE}" in
            release)
                die "[Error] flatbuffers not supported build type."
                ;;
            debug)
                die "[Error] flatbuffers not supported build type."
                ;;
            llt)
                ...
                cmake -G "Unix Makefiles" -DCMAKE_INSTALL_PREFIX=${SCRIPT_PATH}/install_llt ..
                ;;
            release_llt)
                die "[Error] flatbuffers not supported build type."
                ;;
            ...
            comm)
                ...
                cmake -G "Unix Makefiles" -DCMAKE_INSTALL_PREFIX=${SCRIPT_PATH}/install_comm ..
                ;;
            *)
        ...
        if [ $? -ne 0 ]; then
            die "[Error] flatbuffers configure failed."
        fi
        ...
        log_process "[Notice] flatbuffers using \"${COMPILE_TYPE}\" Begin make install"
        make install
        if [ $? -ne 0 ]; then
            die "[Error] flatbuffers make install failed."
        fi
        log_process_done
...
function shrink_component() {
    for COMPILE_TYPE in ${COMPLIE_TYPE_LIST}; do
        ...
        log "[Notice] flatbuffers shrink using \"${COMPILE_TYPE}\" has been finished!"
    done
}
...
function dist_component() {
    for COMPILE_TYPE in ${COMPLIE_TYPE_LIST}; do
        ...
        log "[Notice] flatbuffers dist using \"${COMPILE_TYPE}\" has been finished!"
    done
}
...
function clean_component() {
    ...
    log "[Notice] flatbuffers clean has been finished!"
}
#######################################################################
#######################################################################
#######################################################################

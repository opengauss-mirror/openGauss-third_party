...
#  description: the script that make install ipaddress libs
#  date: 2015-9-15
#  version: 1.0
#  history:
...
LOG_FILE=${LOCAL_DIR}/build_ipaddress.log
BUILD_FAILED=1
TAR_FILE_NAME=ipaddress-1.0.22.tar.gz
SOURCE_CODE_PATH=ipaddress-1.0.22
...
ROOT_DIR="${LOCAL_DIR}/../../../"
INSTALL_COMPOENT_PATH_NAME="${ROOT_DIR}/${COMPONENT_TYPE}/${PLAT_FORM_STR}/install_tools"
...
#######################################################################
log() {
    echo "[Copy ipaddress libs] $(date +%y-%m-%d" "%T): $@"
    echo "[Copy ipaddress libs] $(date +%y-%m-%d" "%T): $@" >> "$LOG_FILE" 2>&1
}
...
function build_component() {
    ...
    for COMPILE_TYPE in ${COMPLIE_TYPE_LIST}; do
        log "[Notice] ipaddress Begin configure..."
        case "${COMPILE_TYPE}" in
            release)
                die "[Error] ipaddress not supported build type."
                ;;
            debug)
                die "[Error] ipaddress not supported build type."
                ;;
            comm)
                log "[Notice] ipaddress build string: python setup.py build"
                mkdir -p ${DEPENDENCY_PATH}/install_comm
				export PYTHONPATH=${DEPENDENCY_PATH}/install_comm
				
                python setup.py build
				if [ $? -ne 0 ]; then
					die "[Error] ipaddress excute cmd python setup.py build failed."
				fi
				
				python setup.py install --install-lib=${DEPENDENCY_PATH}/install_comm
				if [ $? -ne 0 ]; then
					die "[Error] ipaddress python setup.py install --install-lib=${DEPENDENCY_PATH}/install_comm."
				fi
                ;;
            release_llt)
                die "[Error] ipaddress not supported build type."
        ...
        if [ $? -ne 0 ]; then
            die "[Error] ipaddress libs copy failed."
        fi
        log "[Notice] ipaddress libs End copy"
    done
}
...
function dist_component() {
    for COMPILE_TYPE in ${COMPLIE_TYPE_LIST}; do
        case "${COMPILE_TYPE}" in
            ...
            comm) 
                rm -f ${INSTALL_COMPOENT_PATH_NAME}/ipaddress.py
                cp -f ${LOCAL_DIR}/${SOURCE_CODE_PATH}/build/lib/ipaddress.py ${INSTALL_COMPOENT_PATH_NAME}/
                if [ $? -ne 0 ]; then
                    die "[Error] \"cp -f ${LOCAL_DIR}/${SOURCE_CODE_PATH}/build/lib/ipaddress.py ${INSTALL_COMPOENT_PATH_NAME}/\" failed."
                fi
                ;;
        ...
        log "[Notice] ipaddress libs dist using \"${COMPILE_TYPE}\" has been finished!"
    done
}
...
function clean_component() {
    ...
    log "[Notice] ipaddress libs clean has been finished!"
}
#######################################################################
#######################################################################
#######################################################################

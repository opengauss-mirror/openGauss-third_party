...
#  description: the script that make install six libs
#  date: 2015-9-15
#  version: 1.0
#  history:
...
LOG_FILE=${LOCAL_DIR}/build_six.log
BUILD_FAILED=1
TAR_FILE_NAME=six-1.14.0.tar.gz
SOURCE_CODE_PATH=six-1.14.0
...
#######################################################################
log() {
    echo "[Copy six libs] $(date +%y-%m-%d" "%T): $@"
    echo "[Copy six libs] $(date +%y-%m-%d" "%T): $@" >> "$LOG_FILE" 2>&1
}
...
function build_component() {
    ...
    for COMPILE_TYPE in ${COMPLIE_TYPE_LIST}; do
        log "[Notice] six Begin configure..."
        case "${COMPILE_TYPE}" in
            release)
                die "[Error] six not supported build type."
                ;;
            debug)
                die "[Error] six not supported build type."
                ;;
            comm)
                log "[Notice] six build string: python setup.py build"
                mkdir -p ${DEPENDENCY_PATH}/install_comm
				export PYTHONPATH=${DEPENDENCY_PATH}/install_comm
				
                python setup.py build
				if [ $? -ne 0 ]; then
					die "[Error] six excute cmd python setup.py build failed."
				fi
				
				python setup.py install --install-lib=${DEPENDENCY_PATH}/install_comm
				if [ $? -ne 0 ]; then
					die "[Error] six python setup.py install --install-lib=${DEPENDENCY_PATH}/install_comm."
				fi
                ;;
            release_llt)
                die "[Error] six not supported build type."
        ...
        if [ $? -ne 0 ]; then
            die "[Error] six libs copy failed."
        fi
        log "[Notice] six libs End copy"
    done
}
    ...
    for COMPILE_TYPE in ${COMPLIE_TYPE_LIST}; do
        case "${COMPILE_TYPE}" in
            ...
            comm) 
                ...
                rm -f ${INSTALL_COMPOENT_PATH_NAME}/six.py
                cp -f ${LOCAL_DIR}/${SOURCE_CODE_PATH}/build/lib/six.py ${INSTALL_COMPOENT_PATH_NAME}/
                if [ $? -ne 0 ]; then
                    die "[Error] \"cp -f ${LOCAL_DIR}/${SOURCE_CODE_PATH}/build/lib/six.py ${INSTALL_COMPOENT_PATH_NAME}/\" failed."
                fi
                ;;
        ...
        log "[Notice] six libs dist using \"${COMPILE_TYPE}\" has been finished!"
    done
}
...
function clean_component() {
    ...
    log "[Notice] six libs clean has been finished!"
}
#######################################################################
#######################################################################
#######################################################################
